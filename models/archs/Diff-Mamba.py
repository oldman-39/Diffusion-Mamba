...
from basicsr.models.archs.model import FeedForward
from basicsr.models.archs.vmamba import VSSM, SS2D
...
class DTB(nn.Module):
    ...
    def __init__(self, dim, num_heads, ffn_factor, bias, LayerNorm_type, input_channels=3,
                 num_classes=1,
                 drop_path: float = 0,
                 depths=[2, 2, 9, 2],
                 ...
                 d_state: int = 32, ffn_expansion_factor=2.66,
                 **kwargs,
                 ):
    ...
    def forward(self, x, t):
        ...
        x=x.permute(0,2,3,1)
        t= t.permute(0, 2, 3, 1)
        x = x + self.drop_path(self.self_attention(x+t))
        #x = x + self.attn(x + t)
        #print(x.shape,t.size)
        x=x.permute(0,3,1,2)
        t= t.permute(0,3,1,2)
